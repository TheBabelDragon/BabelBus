...
static const char *const k_nvs_ns = "babelbus";
...
esp_err_t jpeg_quality_save_to_nvs(uint8_t q)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGW("babelbus", "nvs_open %s: %s", k_nvs_ns, esp_err_to_name(err));
        return err;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGW("babelbus", "jpeg_q NVS write: %s", esp_err_to_name(err));
    }
    nvs_close(h);
    return err;
}
